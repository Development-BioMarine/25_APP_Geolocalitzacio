...
server <- function(input, output) {
  ...
  observeEvent(input$myrefcam, {  #Apretar boto
    ...
    #llistat_imatges <-reactive( list.files(path= directori_imatges,pattern = [file://.(JPG|JPEG|PNG)$]\\.(JPG|JPEG|PNG)$, full.names = TRUE))
  ...
  observeEvent(input$geoloc_button,{
    ...
    #Creem la columna dels id

    all_files$id <- seq_len(n_fotos)
    ...
    output$map <- renderLeaflet({
      ...
      leaflet() %>%
        ...
        addProviderTiles(providers$Esri.WorldImagery,group = "Satel.lit") %>%
        addLayersControl(baseGroups = c("Satel.lit","World Street Map"))%>%
        addMarkers(lng= as.numeric(all_files[,6]) , lat= (all_files[,7]),

                   options = markerOptions(draggable = TRUE),
                   popup =  paste ( "Longitud:",  as.character(isolate( all_files[,6])),
                                    "<br>",
                                    "Latitud:",as.character(isolate(all_files[,7]))))
